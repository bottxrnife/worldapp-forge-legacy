...
| `messages` | UI chat: `chat` \| `activity` \| `card` messages; `newChat()` clears messages + `apiHistory` + draft (Create "New chat") |
...
| `homeShortcuts` | Customizable Home tile grid (persisted); `addShortcut` / `removeShortcut` / `moveShortcut(id, ±1)` / `resetShortcuts`; `DEFAULT_SHORTCUTS`. Favoriting a dapp (detail heart) pins it here |
...
| Date | Author | Change |
|---|---|---|
| 2026-06-13 | Build agent | **Live World ID 4.0 app + 5 GitHub issue fixes.** Created the real World app via the developer-portal MCP (team "dApp Dock"): app `app_e642b84ff13c702c62e16c5997d27db5`, RP `rp_3c60d66756b89a0c` (registered on-chain, production+staging), action `verify-human`; wired `EXPO_PUBLIC_WORLD_APP_ID/_RP_ID/_ACTION/_ENV` into `.env` (gitignored) with the RP signer key stored server-only (no `EXPO_PUBLIC_` prefix). **Issues:** (#5) Android hardware back no longer closes the app — `HardwareBack` in `_layout.tsx` pops the stack / falls back to Home, only exits from Home/onboarding. (#4) Home + Store headers are **pinned** (greeting+balance+search / title+search+category pills stay put; body scrolls under them). (#2) Create no longer hides the tab bar when a chat is active — bar stays visible, added a **New chat** button (`store.newChat()`). (#1) Add-to-home sheet backdrop now **fades** while the sheet slides up on its own (was one sliding Modal). (#3) Add sheet gains **search + a Recent row**, the detail **heart now favorites → pins the dapp to Shortcuts**, and edit mode adds **◀ ▶ reorder** (`store.moveShortcut`). `tsc` + iOS bundle clean. |
