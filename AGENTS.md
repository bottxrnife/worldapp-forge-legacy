...
│   ├── components/
│   │   ├── ui.tsx                # Shared UI primitives (Txt, Overline, EmptyState, Screen, buttons, motion)
...
**Model default:** `claude-sonnet-4-6` (override via `EXPO_PUBLIC_ANTHROPIC_MODEL`). The Claude Code proxy (`dappdock-claude-proxy`) also pins Sonnet requests to `claude-sonnet-4-6`.
...
**Conventions:**
- Match existing `src/theme.ts` tokens; don’t invent new colors. `C` is a **proxy over light/dark palettes** — never snapshot `C.x` into module-level constants; read it during render. Text/icons on `C.cta` backgrounds must use `C.ctaText` (never `C.white`). Dark panels use `C.inkPanel` (`C.blueInk` is text-on-blueSoft and inverts in dark mode). **Text/surfaces drawn on `C.inkPanel` use the `C.onInk*` tokens (`onInk`/`onInkLabel`/`onInkDim`/`onInkFaint`/`onInkChip`/`onInkHair`/`onInkTrack`) — never raw hex like `#B8C6F2`** (the ink panel is dark in both themes, so these tokens are theme-independent). Every new route screen must subscribe via `useApp((s) => s.themeMode)` (or destructure the whole store) so theme toggles repaint it.
- Reuse the shared primitives in `ui.tsx` instead of re-implementing them inline: **`Overline`** (the uppercase, letter-spaced micro-label that captions cards/sections) and **`EmptyState`** (centered icon-tile + title + subtitle + optional action). New corner radii should come from the **`RADIUS`** scale in `theme.ts` (`xs 11 · sm 13 · md 16 · lg 20 · xl 22 · xxl 26 · pill`).
...
| Date | Author | Change |
|---|---|---|
| 2026-06-13 | Design-language agent | **Design-system consolidation pass (no visual regressions).** Audited every screen for design-language inconsistencies and centralized the most-repeated patterns. (1) **On-ink tokens** — the dark-`inkPanel` text/surface colors were raw hex (`#B8C6F2`/`#9FB0DA`/`#8C9BCB` + `rgba(255,255,255,0.12)`) repeated ~20× across 9 files; added `C.onInk`/`onInkLabel`/`onInkDim`/`onInkFaint`/`onInkChip`/`onInkHair`/`onInkTrack` (identical in both palettes since the panel is always dark → zero visual change) and adopted them in profile, wallet, rewards, pay, redpacket ×2, scan, runtime, assistant (ComposerInspector), PunchCard, RestaurantApp. (2) **`Overline` primitive** — the uppercase micro-label was hand-rolled ~21× across 14 files with drifting size/spacing/color; added one canonical `Overline` in `ui.tsx` and adopted it (detail `MicroLabel` now delegates to it; preview, wallet, pay, rewards, profile, RestaurantApp, PunchCard, assistant). (3) **`EmptyState` primitive** — standardized the one-off "nothing here" placeholders (Activity, Store, Search, assistant Flow tab, order history) onto one icon-tile + title + subtitle + optional-action layout. (4) **`RADIUS` scale** added to `theme.ts` to anchor future corner-radius choices. (5) **Visible polish:** Store header gained a subtitle (matches the title+supporting-line pattern used everywhere else); Detail's odd "Details" text button became a circular `Info` icon button consistent with the share/heart controls (testID `detail-info`); removed dead `monoFor` in `rewards.tsx`. `tsc --noEmit` + iOS `expo export` + lint all clean. |
| 2026-06-13 | Build agent | **Default LLM → Claude Sonnet 4.6 + fullscreen composer + proxy label/uptime.** (1) `agent.ts` `MODEL` default `claude-sonnet-4-5` → **`claude-sonnet-4-6`** (override still via `EXPO_PUBLIC_ANTHROPIC_MODEL`); the Claude Code proxy `map_model` now pins Sonnet to `claude-sonnet-4-6`; synced `.env.example` + proxy README examples. (2) **Assistant composer is now fullscreen** — tapping "Describe your dapp" opens a `Modal` with a multiline input and a new `useKeyboardHeight()` hook keeps the Generate button above the keyboard (fixes the input vanishing behind the IME under SDK 54 edge-to-edge). (3) Header/agent labels reworded: **"Using Claude API"** (direct key) / **"Using Claude Code"** (subscription) / "Template mode" — no more "proxy". `tsc` clean. |
